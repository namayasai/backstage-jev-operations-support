...
const styles = `
.jev{--ink:#e9f0f5;--muted:#a6b8c8;--line:#304253;--panel:#172736;--accent:#8fe5cb;background:#0d1924;color:var(--ink);font:15px/1.6 system-ui,sans-serif;min-height:100vh;padding:38px 5%;box-sizing:border-box;color-scheme:dark}
...
@media(max-width:850px){.jev{padding:24px 18px}.jev-grid{grid-template-columns:1fr}.jev-nav{display:grid;grid-template-columns:repeat(2,minmax(0,1fr))}.jev-nav small{display:none}.jev-panel{padding:20px}.jev-top{margin-bottom:24px}.jev-intro{margin-bottom:24px}}
`;

export function JevWorkbench({ evaluate, loadCandidates, renderCandidateLink, initialWorkflow = 'readiness', initialText, demo = false }: WorkbenchProps) {
  ...
  function switchWorkflow(id: WorkflowId) { clearResult(); setWorkflow(id); setText(''); setCandidates([]); setCatalogTerm(''); }
  ...
  function example() {
    clearResult(); setText(sampleText[workflow]);
  ...
  return <main className="jev">
    ...
    <div className="jev-grid">
      <nav className="jev-nav" aria-label="Decision workflows">
        {workflows.map((w, i) => <button key={w.id} aria-pressed={workflow === w.id} disabled={busy || loading} onClick={() => switchWorkflow(w.id)}><span className="jev-num">0{i + 1}</span>{w.title}</button>)}
      ...
      <section aria-label={current.title}>
        <div className="jev-panel">
          ...
          <textarea id="jev-context" value={text} maxLength={16000} placeholder={current.prompt} onChange={e => { clearResult(); setText(e.target.value); }} disabled={busy} />
          <div className="jev-help">{current.prompt} <span className="jev-count">{text.length.toLocaleString()} / 16,000</span></div>
          {needsCandidates && <div className="jev-candidates">
            ...
            {loadCandidates && <><label className="jev-label" htmlFor="jev-catalog-filter">Catalog filter</label><input id="jev-catalog-filter" placeholder="Optional keyword, e.g. payments" value={catalogTerm} onChange={e => setCatalogTerm(e.target.value)} disabled={busy || loading} /></>}
            {candidates.map((c, i) => <div className="jev-candidate" key={c.id}>
              <div className="jev-candidate-fields"><input aria-label={`Candidate ${i + 1} title`} value={c.title} maxLength={200} onChange={e => updateCandidate(i, 'title', e.target.value)} disabled={busy} /><textarea aria-label={`Candidate ${i + 1} description`} value={c.description} maxLength={1500} onChange={e => updateCandidate(i, 'description', e.target.value)} disabled={busy} /></div>
              <button className="jev-quiet" aria-label={`Remove candidate ${i + 1}`} disabled={busy} onClick={() => { clearResult(); setCandidates(candidates.filter((_, j) => i !== j)); }}>×</button>
            </div>)}
            <div className="jev-actions"><button className="jev-secondary" disabled={busy || loading || candidates.length >= 20} onClick={() => { clearResult(); setCandidates([...candidates, { id: crypto.randomUUID(), title: '', description: '' }]); }}>+ Add candidate</button>
              {loadCandidates && <button className="jev-secondary" disabled={busy || loading} onClick={fromCatalog}>{loading ? 'Loading…' : 'Load from catalog'}</button>}</div>
          ...
          <div className="jev-actions"><button className="jev-primary" disabled={busy || loading} onClick={run}>{busy ? 'Evaluating…' : demo ? 'Show example result →' : 'Evaluate with Jev →'}</button><button className="jev-quiet" disabled={busy || loading} onClick={example}>Load example input</button></div>

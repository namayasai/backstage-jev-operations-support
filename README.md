...
- The AWS module is the only part of this project that stores data. Backstage Notifications does not persist `payload.metadata`, so the alarm context and the receive-time Jev result are kept in one table (`jev_aws_alert_details`) in the Backstage database the host already provides to this plugin, with a 30-day retention. Everything else — recipients, inbox identity, read and saved state — stays with standard Notifications, and an alert whose stored details are gone is still shown as an alert.

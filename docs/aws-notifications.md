...
Human reports are entered through the separate **Triage** sidebar page (`/jev-triage`). The Alerts page remains the received-notification list. To generate possible causes, checks and conditional response options after Jev triage, enable the optional [response-planning LLM](response-planning.md). Its output is shown separately from Jev's classification.

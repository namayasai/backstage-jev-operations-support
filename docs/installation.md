...
This release targets the new Backstage backend system, React 18, and the Backstage package versions recorded in `package-lock.json`. The frontend provides both the legacy plugin export and a new frontend `/alpha` export. Use a non-production Backstage instance first.

## 1. Install packages

Download the three tarballs from the [v0.1.0 release](https://github.com/namayasai/backstage-jev/releases/tag/v0.1.0), or build them yourself with `npm ci && npm run pack:plugins`.
...
{
  "resolutions": {
    "@namayasai/backstage-plugin-jev-common": "https://github.com/namayasai/backstage-jev/releases/download/v0.1.0/namayasai-backstage-plugin-jev-common-0.1.0.tgz"
  }
}
```
...
```sh
yarn --cwd packages/app add @namayasai/backstage-plugin-jev@https://github.com/namayasai/backstage-jev/releases/download/v0.1.0/namayasai-backstage-plugin-jev-0.1.0.tgz
yarn --cwd packages/backend add @namayasai/backstage-plugin-jev-backend@https://github.com/namayasai/backstage-jev/releases/download/v0.1.0/namayasai-backstage-plugin-jev-backend-0.1.0.tgz
```
...
```ts
backend.add(import('@namayasai/backstage-plugin-jev-backend'));
```
...
For an explicitly synthetic local demo, omit `apiKey` and set `jev.demoMode: true`. Responses carry `mode: demo` and the frontend displays a fixture warning. Turn it off for real evaluations.
...
```tsx
import jevPlugin from '@namayasai/backstage-plugin-jev/alpha';
...
This adds `/jev` and a **Jev** entity tab. Preserve the app's existing catalog frontend feature, Catalog API, and route bindings. If your feature discovery already installs this plugin, use one registration mechanism rather than installing it twice.
...
```tsx
import { JevPage } from '@namayasai/backstage-plugin-jev';
...
```tsx
<SidebarItem icon={YourIcon} to="jev" text="Jev" />
```

Optionally add an entity tab in `EntityPage.tsx`:

```tsx
import { EntityJevContent } from '@namayasai/backstage-plugin-jev';

<EntityLayout.Route path="/jev" title="Jev">
  <EntityJevContent />
</EntityLayout.Route>
```
...
`POST /api/jev/evaluate`, authenticated via Backstage:
...
`GET /api/jev/status` reports mode and configuration availability without exposing the API key. Both routes remain behind the host's normal backend authentication policy.

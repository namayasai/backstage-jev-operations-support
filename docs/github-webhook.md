...
Every matching document is validated against the shared evaluation limits before the first provider call, so a limit violation never happens after an earlier document was already evaluated. A document that is oversized, empty, or too short for a readiness evaluation refuses the whole delivery with `422` and names that path, without calling Jev at all. GitHub API, Jev, and deadline failures return `503` (or a sanitized provider status) with `retry: manual`. A provider failure can still occur after an earlier document succeeded; the response is then `503` with `partial: true` and the results obtained so far, and the delivery remains eligible for manual redelivery.
